...
async function initTestSequence() {
  try {
    const response = await fetch('./public/Test_Configs/Light_Field_Focus_Test_Sequence.csv');
...
document.getElementById('vrMode').addEventListener('click', async () => {
  ...
  vrReadyMessage.style.backgroundColor = '#000000';
...
function updateCameraPositionUniform() {
  // For desktop view
  if (!renderer.xr.isPresenting) {
    // Only update if we have a valid plane and material
    if (plane && planeMat && planeMat.uniforms && planeMat.uniforms.uCameraPos) {
      const cameraPos = camera.getWorldPosition(new THREE.Vector3());
      if (cameraPos) {
        planeMat.uniforms.uCameraPos.value.copy(cameraPos);
      }
    }
  } 
  // For VR view
